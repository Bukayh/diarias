...
final List<String> _maquinasCaminhoes = [
  ...
  Future<void> _loadSavedData() async {
    ...
    setState(() {
      _nome = prefs.getString('app2_nome') ?? '';
      _maquinaSelecionada = prefs.getString('app2_maquinaSelecionada');
  ...
  Future<void> _saveData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('app2_nome', _nomeController.text);
    if (_maquinaSelecionada != null) {
      await prefs.setString('app2_maquinaSelecionada', _maquinaSelecionada!);
    }
  }
  ...
  Future<void> _loadSavedData() async {
    ...
    setState(() {
      _horimetroInicialController.text =
          prefs.getString('app2_horimetroInicial') ?? '';
      _kmInicialController.text = prefs.getString('app2_kmInicial') ?? '';
      _turnoSelecionado = prefs.getString('app2_turno');
      _dataController.text = prefs.getString('app2_data') ??
          DateFormat('dd/MM/yyyy').format(DateTime.now());
      _horaInicialController.text = prefs.getString('app2_horaInicial') ??
  ...
  Future<void> _saveData() async {
    ...
    if (_horimetroInicialController.text.isNotEmpty) {
      await prefs.setString(
          'app1_horimetroInicial', _horimetroInicialController.text);
    } else {
      await prefs.remove('app2_horimetroInicial'); // Remove se for vazio
    }

    // Salvar KM inicial, garantindo que não seja nulo ou vazio
    if (_kmInicialController.text.isNotEmpty) {
      await prefs.setString('app2_kmInicial', _kmInicialController.text);
    } else {
      await prefs.remove('app2_kmInicial'); // Remove se for vazio
    }

    // Salvar turno (usa valor padrão se não estiver selecionado)
    await prefs.setString('app2_turno',
    ...
    if (_dataController.text.isNotEmpty) {
      await prefs.setString('app2_data', _dataController.text);
    } else {
      await prefs.remove('app2_data'); // Remove se for vazio
    }

    // Salvar hora inicial, garantindo que não seja nulo ou vazio
    if (_horaInicialController.text.isNotEmpty) {
      await prefs.setString('app2_horaInicial', _horaInicialController.text);
    } else {
      await prefs.remove('app2_horaInicial'); // Remove se for vazio
    }
  }
...
class _EscavadeiraScreenState extends State<EscavadeiraScreen> {
  ...
  Future<void> _saveData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('app2_horimetroInicial', _horimetroInicialController.text);
    await prefs.setString('app2_turno', _turnoSelecionado ?? '');
    await prefs.setString('app2_data', _dataController.text);
    await prefs.setString('app2_horaInicial', _horaInicialController.text);
  }
...
class _ViagensScreenState extends State<ViagensScreen> {
  ...
  Future<void> _carregarDados() async {
    ...
    for (var i = 0; i < _viagens.length; i++) {
      setState(() {
        _viagens[i].contador = prefs.getInt('app2_viagem_${i}_contador') ?? 0;
        _viagens[i].maquinaSelecionada =
            prefs.getString('app2_viagem_${i}_maquina');
      });
    }

    // Carregar viagens atípicas
    setState(() {
      _contadorAtipicas = prefs.getInt('app2_contador_atipicas') ?? 0;
    });
      ...
      setState(() {
        _viagensAtipicas.add(ViagemAtipica(
          ...
          observacaoController: TextEditingController(
            text: prefs.getString('app2_viagem_atipica_${i}') ?? '',
          ),
          maquinaSelecionada:
              prefs.getString('app2_viagem_atipica_${i}_maquina'),
        ));
      });
    }
  }
  ...
  Future<void> _salvarDados() async {
    ...
    for (var i = 0; i < _viagens.length; i++) {
      await prefs.setInt('app2_viagem_${i}_contador', _viagens[i].contador);
      ...
      if (_viagens[i].maquinaSelecionada != null &&
        ...
        await prefs.setString(
            'app2_viagem_${i}_maquina', _viagens[i].maquinaSelecionada!);
      } else {
        await prefs.remove(
            'app2_viagem_${i}_maquina'); // Remove se não houver máquina selecionada
      }
    }
    // Salvar viagens atípicas e a máquina selecionada
    await prefs.setInt('app2_contador_atipicas', _contadorAtipicas);
    for (var i = 0; i < _viagensAtipicas.length; i++) {
      await prefs.setString('app2_viagem_atipica_${i}',
      ...
      if (_viagensAtipicas[i].maquinaSelecionada != null &&
        ...
        await prefs.setString('app2_viagem_atipica_${i}_maquina',
            _viagensAtipicas[i].maquinaSelecionada!);
      } else {
        await prefs.remove(
            'app2_viagem_atipica_${i}_maquina'); // Remove se não houver máquina selecionada
      }
    }
  }
...
// Função para salvar os dados no SharedPreferences
  Future<void> _salvarDados2() async {
    ...
    List<String> viagensEncoded = _viagens.map((viagem) {
      return jsonEncode({
        'app2_origem': viagem.origem,
        'app2_destino': viagem.destino,
        'app2_contador': viagem.contador,
        'app2_maquinaSelecionada': viagem.maquinaSelecionada,
      });
    }).toList();
    await prefs.setStringList('app2_viagens_salvas', viagensEncoded);
  }

// Função para carregar os dados do SharedPreferences
  Future<void> _carregarDados2() async {
    final prefs = await SharedPreferences.getInstance();
    List<String>? viagensEncoded = prefs.getStringList('app2_viagens_salvas');
      ...
      setState(() {
        _viagens = viagensEncoded.map((viagemStr) {
          ...
          return ViagemPredefinida(
            origem: viagemMap['app2_origem'],
            destino: viagemMap['app2_destino'],
            contador: viagemMap['app2_contador'],
            maquinaSelecionada: viagemMap['app2_maquinaSelecionada'],
          );
        }).toList();
      });
    }
...
Future<void> _carregarDadosFinalizacao() async {
  ...
  setState(() {
    _horimetroFinalController.text = prefs.getString('app2_horimetroFinal') ?? '';
    _horaFinalController.text = prefs.getString('app2_horaFinal') ?? '';
    _kmFinalController.text = prefs.getString('app2_kmFinal') ?? '';
  });
}  
Future<void> _salvarDadosFinalizacao() async {
  ...
  if (_horimetroFinalController.text.isNotEmpty) {
    await prefs.setString('app2_horimetroFinal', _horimetroFinalController.text);
  } else {
    await prefs.remove('app2_horimetroFinal'); // Remove se for vazio
  }

  // Salvar hora final, garantindo que não seja nulo ou vazio
  if (_horaFinalController.text.isNotEmpty) {
    await prefs.setString('app2_horaFinal', _horaFinalController.text);
  } else {
    await prefs.remove('app2_horaFinal'); // Remove se for vazio
  }

  // Salvar KM final, garantindo que não seja nulo ou vazio
  if (_kmFinalController.text.isNotEmpty) {
    await prefs.setString('app2_kmFinal', _kmFinalController.text);
  } else {
    await prefs.remove('app2_kmFinal'); // Remove se for vazio
  }
}
...
''';
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('app2_dados_viagem', dados);
