...
class AppVersion1 extends StatefulWidget {
  @override
  _AppVersion1State createState() => _AppVersion1State();
}
...
class _AppVersion1State extends State<AppVersion1> {
  ...
  Future<void> _loadSavedData() async {
    ...
    setState(() {
      _nome = prefs.getString('app1_nome') ?? '';
      _maquinaSelecionada = prefs.getString('app1_maquinaSelecionada');
  ...
  Future<void> _saveData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('app1_nome', _nomeController.text);
    if (_maquinaSelecionada != null) {
      await prefs.setString('app1_maquinaSelecionada', _maquinaSelecionada!);
    }
  }
...
class _CaminhaoScreenState extends State<CaminhaoScreen> {
  ...
  Future<void> _loadSavedData() async {
    ...
    setState(() {
      _horimetroInicialController.text =
          prefs.getString('app1_horimetroInicial') ?? '';
      _kmInicialController.text = prefs.getString('app1_kmInicial') ?? '';
      _turnoSelecionado = prefs.getString('app1_turno');
      _dataController.text = prefs.getString('app1_data') ??
          DateFormat('dd/MM/yyyy').format(DateTime.now());
      _horaInicialController.text = prefs.getString('app1_horaInicial') ??
  ...
  Future<void> _saveData() async {
    ...
    if (_horimetroInicialController.text.isNotEmpty) {
      ...
      await prefs.remove('app1_horimetroInicial'); // Remove se for vazio
    }

    // Salvar KM inicial, garantindo que não seja nulo ou vazio
    if (_kmInicialController.text.isNotEmpty) {
      await prefs.setString('app1_kmInicial', _kmInicialController.text);
    } else {
      await prefs.remove('app1_kmInicial'); // Remove se for vazio
    }

    // Salvar turno (usa valor padrão se não estiver selecionado)
    await prefs.setString('app1_turno',
    ...
    if (_dataController.text.isNotEmpty) {
      await prefs.setString('app1_data', _dataController.text);
    } else {
      await prefs.remove('app1_data'); // Remove se for vazio
    }

    // Salvar hora inicial, garantindo que não seja nulo ou vazio
    if (_horaInicialController.text.isNotEmpty) {
      await prefs.setString('app1_horaInicial', _horaInicialController.text);
    } else {
      await prefs.remove('app1_horaInicial'); // Remove se for vazio
    }
  }
...
class _EscavadeiraScreenState extends State<EscavadeiraScreen> {
  ...
  Future<void> _saveData() async {
    ...
    await prefs.setString(
        'app1_horimetroInicial', _horimetroInicialController.text);
    await prefs.setString('app1_turno', _turnoSelecionado ?? '');
    await prefs.setString('app1_data', _dataController.text);
    await prefs.setString('app1_horaInicial', _horaInicialController.text);
  }
...
class _ViagensScreenState extends State<ViagensScreen> {
  ...
  Future<void> _carregarDados() async {
    ...
    for (var i = 0; i < _viagens.length; i++) {
      setState(() {
        _viagens[i].contador = prefs.getInt('app1_viagem_${i}_contador') ?? 0;
        _viagens[i].maquinaSelecionada =
            prefs.getString('app1_viagem_${i}_maquina');
      });
    }

    // Carregar viagens atípicas
    setState(() {
      _contadorAtipicas = prefs.getInt('app1_contador_atipicas') ?? 0;
    });
      ...
      setState(() {
        _viagensAtipicas.add(ViagemAtipica(
          ...
          observacaoController: TextEditingController(
            text: prefs.getString('app1_viagem_atipica_${i}') ?? '',
          ),
          maquinaSelecionada:
              prefs.getString('app1_viagem_atipica_${i}_maquina'),
        ));
      });
    }
  }
  ...
  Future<void> _salvarDados() async {
    ...
    for (var i = 0; i < _viagens.length; i++) {
      await prefs.setInt('app1_viagem_${i}_contador', _viagens[i].contador);
      ...
      if (_viagens[i].maquinaSelecionada != null &&
        ...
        await prefs.setString(
            'app1_viagem_${i}_maquina', _viagens[i].maquinaSelecionada!);
      } else {
        await prefs.remove(
            'app1_viagem_${i}_maquina'); // Remove se não houver máquina selecionada
      }
    }
    // Salvar viagens atípicas e a máquina selecionada
    await prefs.setInt('app1_contador_atipicas', _contadorAtipicas);
    for (var i = 0; i < _viagensAtipicas.length; i++) {
      await prefs.setString('app1_viagem_atipica_${i}',
      ...
      if (_viagensAtipicas[i].maquinaSelecionada != null &&
        ...
        await prefs.setString('app1_viagem_atipica_${i}_maquina',
            _viagensAtipicas[i].maquinaSelecionada!);
      } else {
        await prefs.remove(
            'app1_viagem_atipica_${i}_maquina'); // Remove se não houver máquina selecionada
      }
    }
  }
...
// Função para salvar os dados no SharedPreferences
  Future<void> _salvarDados2() async {
    ...
    List<String> viagensEncoded = _viagens.map((viagem) {
      return jsonEncode({
        'app1_origem': viagem.origem,
        'app1_destino': viagem.destino,
        'app1_contador': viagem.contador,
        'app1_maquinaSelecionada': viagem.maquinaSelecionada,
      });
    }).toList();
    await prefs.setStringList('app1_viagens_salvas', viagensEncoded);
  }

// Função para carregar os dados do SharedPreferences
  Future<void> _carregarDados2() async {
    final prefs = await SharedPreferences.getInstance();
    List<String>? viagensEncoded = prefs.getStringList('app1_viagens_salvas');
      ...
      setState(() {
        _viagens = viagensEncoded.map((viagemStr) {
          ...
          return ViagemPredefinida(
            origem: viagemMap['app1_origem'],
            destino: viagemMap['app1_destino'],
            contador: viagemMap['app1_contador'],
            maquinaSelecionada: viagemMap['app1_maquinaSelecionada'],
          );
        }).toList();
      });
    }
...
Future<void> _carregarDadosFinalizacao() async {
  ...
  setState(() {
    _horimetroFinalController.text = prefs.getString('app1_horimetroFinal') ?? '';
    _horaFinalController.text = prefs.getString('app1_horaFinal') ?? '';
    _kmFinalController.text = prefs.getString('app1_kmFinal') ?? '';
  });
}  
Future<void> _salvarDadosFinalizacao() async {
  ...
  if (_horimetroFinalController.text.isNotEmpty) {
    await prefs.setString('app1_horimetroFinal', _horimetroFinalController.text);
  } else {
    await prefs.remove('app1_horimetroFinal'); // Remove se for vazio
  }

  // Salvar hora final, garantindo que não seja nulo ou vazio
  if (_horaFinalController.text.isNotEmpty) {
    await prefs.setString('app1_horaFinal', _horaFinalController.text);
  } else {
    await prefs.remove('app1_horaFinal'); // Remove se for vazio
  }

  // Salvar KM final, garantindo que não seja nulo ou vazio
  if (_kmFinalController.text.isNotEmpty) {
    await prefs.setString('app1_kmFinal', _kmFinalController.text);
  } else {
    await prefs.remove('app1_kmFinal'); // Remove se for vazio
  }
}
...
''';
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('app1_dados_viagem', dados);
